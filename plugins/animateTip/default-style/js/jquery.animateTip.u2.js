...
(function ($) {
	$.fn.animateTip = function (options) {
		var defaults = {
			...
			animateType:"slide"	//slide,fade,show
		};
		setOptions(options);
		...
		function animateFun(panel){
			...
			var ww=$(window).width(),
				wh=$(window).height();
			if(panel.find(".animate-tip-arrow").hasClass("bottom")){
			...
			}else if(defaults.animateType=="fade"){
				panel.fadeIn(defaults.delay);
			}else{
				panel.show();
			}
		}
		...
		function animateClearFun(panel){
			...
			if(defaults.animateType=="slide"){
				...
				panel.fadeOut(defaults.delay,function(){
					panel.remove();
				});
			}else{
				panel.remove();
